...
export type ImportBibleFromEpubInput = { epub: EpubBibleImportInput; importer: EpubBibleImporter; repository: ObsidianBibleIndexV2Repository; };
...
export async function importBibleFromEpub(input: ImportBibleFromEpubInput): Promise<EpubBibleImportResult> {
    ...
    await input.repository.saveV2({ metadata: result.bibleIndexV2Data, books: result.compactBooks, report: result.report });
    return result;
}
